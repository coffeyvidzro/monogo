...
CommPeak supplies Leamout-managed SIP termination and origination; DIDWW supplies managed DID inventory, acquisition, and inbound PSTN routing.
...
2. Use the seeded active `commpeak` carrier provider and existing internal platform operations to create a platform-scoped connection (organization ID NULL), and a platform-managed outbound trunk. Start disabled. Set `managed_default` only after verifying connectivity, authentication and commercial authorization.
...
6. Confirm that the platform outbound credential view used by OpenSIPS exists and contains the expected scoped, non-plaintext auth material. Exercise real SIP INVITE, challenge, media, source allowlist and negative authorization tests before activation.
