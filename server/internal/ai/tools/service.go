...
func (s *Service) Create(ctx context.Context, organizationID, agentID uuid.UUID, req CreateRequest) (sqlc.VoiceAgentTool, error) {
	if err := validateIDs(organizationID, agentID); err != nil { return sqlc.VoiceAgentTool{}, err }
	normalized, err := normalizeCreate(req); if err != nil { return sqlc.VoiceAgentTool{}, err }
	tool, err := s.repo.Create(ctx, organizationID, agentID, normalized)
	if conflict(err) { return sqlc.VoiceAgentTool{}, apperror.NewConflict("voice agent tool already exists") }
	if err != nil { return sqlc.VoiceAgentTool{}, dbError(err, "create voice agent tool") }
	return tool, nil
}
...
func (s *Service) List(ctx context.Context, organizationID, agentID uuid.UUID) ([]sqlc.VoiceAgentTool, error) {
	...
	if err != nil { return nil, apperror.NewInternal("list voice agent tools", err) }
	return items, nil
}
...
func (s *Service) Update(ctx context.Context, organizationID, agentID, id uuid.UUID, req UpdateRequest) (sqlc.VoiceAgentTool, error) {
	if err := validateIDs(organizationID, agentID); err != nil { return sqlc.VoiceAgentTool{}, err }
	if id == uuid.Nil { return sqlc.VoiceAgentTool{}, apperror.NewBadRequest("tool id is required") }
	normalized, err := normalizeUpdate(req); if err != nil { return sqlc.VoiceAgentTool{}, err }
...
func (s *Service) Delete(ctx context.Context, organizationID, agentID, id uuid.UUID) error {
	...
	if id == uuid.Nil { return apperror.NewBadRequest("tool id is required") }
