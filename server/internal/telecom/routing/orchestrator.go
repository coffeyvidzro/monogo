...
func validateOrchestrationPolicy(policy OrchestrationPolicy) error {
	...
	if !percentage(policy.MinASR) || !percentage(policy.MaxPacketLossPercent) || policy.MinALOCSeconds < 0 {
...
func carrierScore(policy OrchestrationPolicy, candidate CarrierCandidate) (int64, error) {
	...
	aloc := 0.0
	if policy.MinALOCSeconds > 0 {
		aloc = math.Max(0, policy.MinALOCSeconds-candidate.ALOCSeconds) / policy.MinALOCSeconds
	}
