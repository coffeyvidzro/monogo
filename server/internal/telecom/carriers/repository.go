...
func (r *Repository) List(ctx context.Context, org uuid.UUID) ([]sqlc.ListCarrierConnectionsByOrganizationIDRow, error) {
...
func (r *Repository) SetInboundDigest(ctx context.Context, org, id uuid.UUID, username, ciphertext string) error {
	return r.queries.SetCarrierConnectionInboundDigestAuth(ctx, sqlc.SetCarrierConnectionInboundDigestAuthParams{InboundUsername: &username, InboundSecretCiphertext: &ciphertext, ID: id, OrganizationID: &org})
}
...
func (r *Repository) ListSourceIPs(ctx context.Context, org, id uuid.UUID) ([]sqlc.CarrierConnectionSourceIp, error) {
	return r.queries.ListCarrierConnectionSourceIPs(ctx, sqlc.ListCarrierConnectionSourceIPsParams{CarrierConnectionID: id, OrganizationID: &org})
}
func (r *Repository) DeleteSourceIP(ctx context.Context, org, id, sourceID uuid.UUID) error {
	return r.queries.DeleteCarrierConnectionSourceIP(ctx, sqlc.DeleteCarrierConnectionSourceIPParams{ID: sourceID, CarrierConnectionID: id, OrganizationID: &org})
}
